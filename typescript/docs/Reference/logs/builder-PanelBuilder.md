...
```typescript
dedupStrategy(dedupStrategy: common.LogsDedupStrategy)
```
...
```typescript
prettifyLogMessage(prettifyLogMessage: boolean)
```
...
```typescript
showLogContextToggle(showLogContextToggle: boolean)
```
...
```typescript
wrapLogMessage(wrapLogMessage: boolean)
```
